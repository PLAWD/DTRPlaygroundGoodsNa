...
// Clean implementation of save function - completely bypasses DOM to avoid warnings
document.getElementById('save-info').addEventListener('click', function() {
  ...
  // Temporarily hide any warning or review elements
  const warningElements = recordBox.querySelectorAll('.warning-icon, .warning-text, .review-row, .issues-header');
  warningElements.forEach(el => { el.style.display = 'none'; });
  
  // Get only record entries that don't contain warning elements
  const recordEntries = Array.from(recordBox.querySelectorAll('.record-entry')).filter(entry => 
    !entry.classList.contains('review-row')
  );
  
  const records = [];
  
  // Process each valid record entry
  recordEntries.forEach(entry => {
    const textElement = entry.querySelector('.record-text');
    if (textElement) {
      const text = textElement.textContent.trim();
      // Very strict validation for properly formatted records only
      if (text.match(/^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday) - \d{2}\/\d{2}\/\d{4} - \d{1,2}:\d{2} (AM|PM)$/)) {
        records.push(text);
      }
    }
  });
  
  // Extra validation to ensure no warning messages slipped through
  const cleanRecords = records.filter(record => 
    !record.includes("⚠️") && 
    !record.includes("Marked for Review") &&
    !record.includes("Issues found") &&
    !record.includes("Time In") &&
    !record.includes("Time Out") &&
    !record.endsWith(")")
  );
  
  // Create clean JSON with only validated records
  const jsonData = { "recordedTimes": cleanRecords };
  ...
  // Restore visibility of any elements we hid
  warningElements.forEach(el => { el.style.display = ''; });
});
    ...
    fetch('/upload', { method: 'POST', body: formData })
      ...
      .then(data => {
        if (data.status === 'success') {
          // Uncheck all logic checkboxes when new records are uploaded
          if (data.uncheck_logics) {
          ...
          const recordBox = document.getElementById('record-box');
          recordBox.innerHTML = "";
          if (data.content.recordedTimes) {
            data.content.recordedTimes.forEach(record => {
              if (typeof record === 'string' && record.trim() !== "") {
                recordBox.appendChild(createRecordEntry(record.trim()));
              }
            });
          }
          
          // If schedules are included, update the schedule list
          if (data.content.schedules) {
            // Update global schedules variable
            window.schedules = data.content.schedules;
            // Call the updateScheduleList function if it exists
            if (typeof updateScheduleList === 'function') {
              updateScheduleList();
            }
          }
        } else {
...
document.getElementById('clear-info').addEventListener('click', function() {
  ...
  // Uncheck all logic checkboxes when records are cleared
  const logicCheckboxes = document.querySelectorAll('#logic1, #logic2, #logic3');
  logicCheckboxes.forEach(checkbox => {
    checkbox.checked = false;
  });
